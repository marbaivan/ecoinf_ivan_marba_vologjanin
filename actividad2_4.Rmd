...
waypoints <- as_tibble(gpx$waypoints) %>%  #convertir a tibble la part de la llista que ens interessa
  slice(-c(10, 11)) %>%  #elimino dues files que representen al mateix arbre
  rename(GPS = Name) %>%          # canviar nom columna
  mutate(GPS = as.numeric(GPS))   # treure zero inicial

data <- data %>%
  mutate(GPS = as.numeric(sub("-.*", "", GPS))) #convertir columna GPS a numeric i convertir el 49-51 en només 40

data_waypoints <- waypoints %>% 
  left_join(data, by = "GPS") %>% 
  select(Elevation, Latitude, Longitude, GPS, STATUS)%>%  #em quedo amb les columnes que m'interessen
  mutate(
    STATUS = factor(
      STATUS,
      levels = c("A", "D"),
      labels = c("Alive", "Dead")  #canvi per a que a la llegenda del mapa surti Alive i Dead
    )
  ) 


pal <- colorFactor(            #paleta colors
  palette = c("green", "red"),
  domain = data_waypoints$STATUS
) 
...
leaflet(data = data_waypoints) %>%
  ...
  addLegend(
    ...
    values = ~STATUS,
    title = "Tree status",
    labels = c("Viu (A)", "Mort (D)")
  ) %>% 
  addScaleBar(position = "bottomleft", 
              options = scaleBarOptions(imperial = F)) %>% #escala
  addControl(north_arrow, position = "topleft")   #north arrow
